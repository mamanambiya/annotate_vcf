...
parser.add_argument("--inVCF", default="${inVCF}", help="cpmpressed VCF file (gz) to annotate")
...
def readVCF(inTSV, inVCF, outVCF):
    ...
    :return:
    '''
    out = gzip.open(outVCF, 'wb')
    ann = readTSV(inTSV)
    for line in gzip.open(inVCF, 'rb'):
...
if __name__ == '__main__':
    if args.inTSV != '' and args.inVCF != '' and args.outVCF != '':
